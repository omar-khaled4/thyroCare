...
export default function SignUp(){
    ...
    let validationSchema = yup.object().shape({
        firstName : yup.string().min(3, " min lenght is 3 char ").max(25 , " max lenght is 25 char ").required(" First name is required ") ,
        lastName : yup.string().min(3, " min lenght is 3 char ").max(25 , " max lenght is 25 char ").required(" Last name is required ") ,
            ...
            <div className="pt-35 mx-10 grid gap-4 md:grid-cols-12">
                ...
                <div className="md:col-span-6 md:col-start-7 lg:col-span-5 lg:col-start-8">
                    <div className="w-full backdrop-blur-[10px] bg-white/10 rounded-[50px] border border-white mb-4">
                        <p className="text-white font-1 text-center text-2xl py-3 border-b-2 mx-5">Sign Up</p>
                    
                        <form onSubmit={formik.handleSubmit} className="m-5 ">
                            <div className="grid gap-1 grid-cols-2">
                                <div>
                                    <label htmlFor="FName" className="text-white font-1 w-full text-lg">First Name</label>
                                    <input type="text" id="FName" name="firstName" value={formik.values.firstName} onChange={formik.handleChange} onBlur={formik.handleBlur} className="w-full font-1 bg-[#00000000] text-white rounded-lg" required/>
                                    { formik.errors.firstName && formik.touched.firstName ?( <p className="font-1 pt-1 text-red-800">{formik.errors.firstName}</p> ):null}
                                </div>
                                <div>
                                    <label htmlFor="LName" className="text-white font-1 w-full text-lg">Last Name</label>
                                    <input type="text" id="LName" name="lastName" value={formik.values.lastName} onChange={formik.handleChange} onBlur={formik.handleBlur} className="w-full font-1 bg-[#00000000] text-white rounded-lg" required/>
                                    { formik.errors.lastName && formik.touched.lastName ?( <p className="font-1 pt-1 text-red-800">{formik.errors.lastName}</p> ):null}
                                </div>
                            </div>
                            <div className="mt-4">
                            ...
                            <div className="mt-4">
                                <label htmlFor="phone" className="text-white font-1 w-full text-lg">Phone</label>
                                <input type="tel" id="phone" name="phone" value={formik.values.phone} onChange={formik.handleChange} onBlur={formik.handleBlur} className="w-full font-1 bg-[#00000000] text-white rounded-lg" placeholder="01000000000" required/>
                                { formik.errors.phone && formik.touched.phone ?( <p className="font-1 pt-1 text-red-800">{formik.errors.phone}</p> ):null}
                            </div>
                            <div className="mt-4">
                            ...
                            <div className="grid gap-1 grid-cols-2 mt-4">
                                <div>
                                    <label htmlFor="date" className="text-white font-1 w-full text-lg">Date of Birth</label>
                                    <input type="date" id="date" name="dateOfBirth" value={formik.values.dateOfBirth} onChange={formik.handleChange} onBlur={formik.handleBlur} className="w-full font-1 bg-[#00000000] text-white rounded-lg" required/>
                                    { formik.errors.dateOfBirth && formik.touched.dateOfBirth ?( <p className="font-1 pt-1 text-red-800">{formik.errors.dateOfBirth}</p> ):null}
                                </div>
                                <div>
                                    <label htmlFor="gender" className="text-white font-1 w-full text-lg">Gender</label>
                                    <select id="gender" name="gender" value={formik.values.gender} onChange={formik.handleChange} onBlur={formik.handleBlur} className="w-full font-1 bg-[#00000000] text-white rounded-lg" required>
